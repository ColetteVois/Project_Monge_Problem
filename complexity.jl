...
function complexity()
  ...
  time_N = zeros(length(N_tab))
  time_tol = zeros(length(tol_tab))
  ...
  #deplacement top
  moving_top = Array{Array{Float64,1}}(1, length(N_tab), 1)
  for i in 1:length(N_tab)
  ...
  #deplacement right
  moving_right = Array{Array{Float64,1}}(1, length(N_tab), 1)
  for i in 1:length(N_tab)
      moving_right[i] = main(N_tab[i],tol_tab[5],[0,1],1)[2:3]
  end

  #deplacement diagonal
  moving_diag = Array{Array{Float64,1}}(1, length(N_tab), 1)
  for i in 1:length(N_tab)
